...
function seedprod_pro_norm_str( $string ) {
	return trim(
		strtolower(
			str_replace( '.', '', $string )
		)
...
function seedprod_pro_in_array_norm( $needle, $haystack ) {
	return in_array( seedprod_pro_norm_str( $needle ), $haystack );
}

function seedprod_pro_parse_name( $fullname ) {
	...
	switch ( $n_pieces ) {
		case 1:  // array(title first middles last suffix)
			...
			for ( $i = 0; $i < $n_subp; $i++ ) {
				...
				if ( $i == 0 && seedprod_pro_in_array_norm( $curr, $titles ) ) {
					$out['title'] = $curr;
					continue;
				}
				...
				if ( $i == $n_subp - 2 && $next && seedprod_pro_in_array_norm( $next, $suffices ) ) {
					if ( $out['last'] ) {
						$out['last'] .= " $curr";
					} else {
						$out['last'] = $curr;
				...
				if ( seedprod_pro_in_array_norm( $curr, $prefices ) ) {
					if ( $out['last'] ) {
						$out['last'] .= " $curr";
					} else {
						$out['last'] = $curr;
			...
			break;
		case 2:
			switch ( seedprod_pro_in_array_norm( $pieces[1], $suffices ) ) {
				...
				case false: // array(last,title first middles suffix)
					...
					for ( $i = 0; $i < $n_subp; $i++ ) {
						...
						if ( $i == $n_subp - 2 && $next &&
						seedprod_pro_in_array_norm( $next, $suffices ) ) {
							if ( $out['middle'] ) {
								$out['middle'] .= " $curr";
							} else {
								$out['middle'] = $curr;
						...
						if ( $i == $n_subp - 1 && seedprod_pro_in_array_norm( $curr, $suffices ) ) {
							$out['suffix'] = $curr;
							continue;
						}
